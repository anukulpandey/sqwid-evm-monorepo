...
const normalizeCollection = item => ({
	...
	thumbnail: getInfuraURL(item?.thumbnail || item?.thumb || item?.image || ""),
	stats: {
		items: item?.stats?.items || 0,
		itemsSold: item?.stats?.itemsSold || 0,
		volume: item?.stats?.volume || 0,
		average: item?.stats?.average || 0,
	},
});
...
const CollectionsPage = () => {
	...
	useEffect (() => {
		const fetchCollections = async () => {
			...
			const data = (res.collections || []).map(normalizeCollection);
			setCollections (data);
		}
		fetchCollections ();
	}, [defaultValue])
