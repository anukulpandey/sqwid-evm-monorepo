...
const Balance = () => {
	...
	return (
		<BalanceWrapper>
			<h4>Balance</h4>
			<BalanceContainer>
				<span>{getNetworkConfig().currencySymbol}</span>{" "}
				<span>{isLoading ? <FadeLoaderIcon /> : balance}</span>
			</BalanceContainer>
		</BalanceWrapper>
	);
};
...
const Withdraw = () => {
	...
	useEffect(() => {
		...
		const fetchPrice = async () => {
			...
			try {
				...
				if (!ignore) {
					setPrice(Number(availableBalance || 0));
				}
			} catch (_error) {
				if (!ignore) {
					setPrice(0);
	...
	const handleWithdraw = async () => {
		...
		try {
			...
			if (
				err.message.includes("No Reef to be claimed") ||
				err.message.includes("No balance")
			) {
				showErrorModal("No balance available to withdraw.");
			} else {
				showErrorModal(err);
			}
		} finally {
