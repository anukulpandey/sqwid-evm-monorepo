...
const safeParseCollections = value => {
	try {
...
const normalizeCollection = collection => {
	...
	return {
		id: collection?.id || data.id || "",
		name: data.name || "",
		description: data.description || "",
		image: data.image || "",
		thumbnail: data.thumbnail || "",
		owner: data.owner || collection?.owner || "",
	};
};

const readStoredCollections = () =>
	safeParseCollections(localStorage.getItem("collections")).map(
		normalizeCollection
	);
...
const New = ({ isActive, setIsActive }) => {
	...
	const handleClick = async () => {
		...
		if (!info.file || !info.name.length) {
			showErrorModal("Please add a collection name and cover image.");
			return;
		}
		...
		try {
			...
			let normalizedCollection = normalizeCollection(createdCollection);

			try {
				const response = await axios.get(
					`${getBackend()}/get/collections/id/${createdCollection.id}`
				);
				normalizedCollection = normalizeCollection(response.data);
			} catch (_error) {
			...
			const nextCollections = [
				normalizedCollection,
				...readStoredCollections().filter(
					item => item.id !== normalizedCollection.id
				),
			];
			localStorage.setItem("collections", JSON.stringify(nextCollections));
			setFiles({
				...files,
				collection: normalizedCollection.id,
				collectionName: normalizedCollection.name,
			});
			setInfo(initialInfo);
			setFileURL("");
...
const Existing = ({ isActive, setIsActive }) => {
	...
	useEffect(() => {
		axios
			...
			.then(res => {
				const nextCollections = (res.data.collections || []).map(
					normalizeCollection
				);
					...
					<>
						{collections.length ? (
							collections.map(item => (
								...
								>
									<img
										src={getInfuraURL(item.thumbnail || item.image)}
