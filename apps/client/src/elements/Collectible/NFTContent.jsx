...
const WarningTextContainer = styled.div`
	...
	box-shadow: rgba(0, 0, 0, 0.1) 0px 10px 15px -3px,
		...
		inset rgba(0, 0, 0, 0.05) 0px 10px 15px -3px,
		inset rgba(0, 0, 0, 0.025) 0px 4px 6px -2px;
	p {
...
const ImageContainer = ({ title, isBlurred, setIsBlurred, ...props }) => {
	...
	const handleError = () => {
		setLoading(false);
		if (props.fallbackSrc && props.fallbackSrc !== src) {
...
const WarningText = ({ isBlurred, setIsBlurred }) => {
	...
	const rejectedText = (
		<>
			<p>
				This item has been automatically rejected by{" "}
				{capitalize(constants.APP_NAME)} Automod.
			</p>
			<p>
				If you're the creator and you think we made a mistake, please
				let us know by going to the <code>Details</code> tab and
				clicking the <code>Appeal</code> button.
			</p>
			<p>
				{" "}
				The feedback you provide us will help us improve the ways we
				keep {capitalize(constants.APP_NAME)} safe.
			</p>
		</>
	);
...
const NFTContentSection = () => {
	...
	const mimetype = collectibleInfo?.meta?.mimetype || "";
	const fallbackImage = collectibleInfo?.meta?.image || collectibleInfo?.meta?.media || "";
	const displayName = collectibleInfo?.meta?.name || `Collectible #${collectibleInfo?.positionId || ""}`.trim();
	const shouldRenderImage = !mimetype || mimetype.startsWith("image");
			...
			{shouldRenderImage ? (
				<>
					<ImageContainer
						...
						isBlurred={isBlurred}
						setIsBlurred={setIsBlurred}
						src={getInfuraURL(fallbackImage)}
						fallbackSrc={getInfuraURL(
							collectibleInfo?.meta?.media || fallbackImage
						)}
					/>
					<ModalComponent
