...
export const REEF_ADDRESS_SPECIFIC_STRING = "";
			...
			<Group>
				<InputWrapper>
					...
					<InputContainer
						...
						placeholder={`Minimum bid for the lot in native token`}
					/>
					<InputTitle>
						Number of editions
						<span>
			...
			<Group>
				<InputWrapper>
					...
					<InputContainer
						...
						placeholder={`Amount to borrow in native token`}
					/>
					<InputTitle>
						Payback Fee{" "}
						<span>
					...
					<InputContainer
						...
						placeholder={`Interest to pay in native token`}
					/>
					<InputTitle>
						Number of editions
						<span>
	...
	const handleClick = async () => {
		if (!isLoading && Number(amount) >= 1) {
			...
			if (receipt) {
				...
				setAmount("");
				props.setIsActive(false);
				bread(`You've added ${amount} native tokens to the raffle!`);
				setTimeout(() => {
					window.location.reload();
				}, 3000);
			} else {
		...
		<ModalContainer {...props}>
			...
			<Group>
				...
				<InputContainer
					...
					value={amount}
					onChange={e => setAmount(e.target.value)}
					placeholder={`Amount to be sent in native token`}
				/>
				<AnimBtn disabled={isLoading} onClick={handleClick}>
					{buttonText}
				</AnimBtn>
	...
	const handleClick = async () => {
		if (!isLoading && Number(amount) + currentBid >= minToBid) {
			...
			if (receipt) {
				...
				setAmount("");
				props.setIsActive(false);
				bread(`You bid ${amount} native tokens!`);
				setTimeout(() => {
					window.location.reload();
				}, 3000);
			} else {
		...
		<ModalContainer {...props}>
			...
			<Group>
				...
				<InputContainer
					...
					value={amount}
					onChange={handleAmountInput}
					placeholder={`Amount to bid in native token`}
				/>
				<AnimBtn disabled={isLoading} onClick={handleClick}>
					{buttonText}
				</AnimBtn>
...
const ItemSelector = ({ item, selectedItem, setSelectedItem, isLoading }) => {
	...
	return (
		<SelectorWrapper active = {isActive} onClick = {handleClick}>
			<SelectorImage src = {getInfuraURL (item.thumbnail || item.thumb || item.image)} alt = {item.name} />
			<div>{item.name}</div>
		</SelectorWrapper>
	);
};
...
export const PickCollectionModal = props => {
	...
	useEffect (() => {
		if (selectedItem) {
			const selectedCollection = collections.find(item => item.id === selectedItem);
			setButtonText(
				`Move collectible to ${selectedCollection?.name || "selected collection"}`
			);
		}
