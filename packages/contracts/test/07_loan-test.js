...
describe("************ Loans ******************", () => {
    ...
    before(async () => {
        ...
        borrower = signers.account2;
        lender = signers.account3;
        const artist = signers.account4;
    ...
    it("Should create loan proposal", async () => {
        ...
        expect(Number(loan.state)).to.equal(4); // Loan = 4
    ...
    it("Should fund loan", async () => {
        ...
        deadline = Number(loan.loanData.deadline);
        ...
        expect(deadline).to.be.lt(now + 90).and.gt(now + 30); // +/- 30 secs. margin
