...
describe("************ Auctions ******************", () => {
    ...
    it("Should create auction", async () => {
        ...
        deadline = Number(auction.auctionData.deadline);
        ...
        expect(Number(auction.state)).to.equal(2); // PositionState.Auction = 2
        expect(deadline).to.be.lt(now + 60 * (numMinutes + 1)).and.gt(now);
    ...
    it("Should create bid", async () => {
        ...
        // Evaluate results
        expect(deadline).equals(oldDeadline);
    ...
    it("Should increase for non-highest bidder and extend auction deadline", async () => {
        ...
        console.log(`\ttime until deadline: ${timeUntilDeadline / 60} mins.`);
        if (timeUntilDeadline > 600) {
            const timeToWait = timeUntilDeadline - 590;
            console.log(`\tadvancing ${timeToWait} seconds...`);
        ...
        const auctionData = (await marketUtil.fetchPosition(auctionId)).auctionData;
        deadline = Number(auctionData.deadline);
        console.log(`\tdeadline extended by ${deadline - oldDeadline} secs.`);

        // Evaluate results
        expect(deadline).gt(oldDeadline);
