const { getMainContracts, getNamedSigners, throwsException } = require("./util");
...
describe("************ Market Util ******************", () => {
    before(async () => {
        ...
        const contracts = await getMainContracts(marketFee, signers.account1);
    ...
    it("Should get positions from verified positions", async () => {
        ...
        if (Number(numItems) === 0 || Number(numPositions) === 0) {
            return;
        }
