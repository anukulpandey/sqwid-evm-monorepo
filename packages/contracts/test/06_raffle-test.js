...
describe("************ Raffles ******************", () => {
    ...
    before(async () => {
        ...
        owner = signers.account1;
        seller = signers.account2;
        buyer1 = signers.account3;
        buyer2 = signers.account4;
        artist = signers.account5;
        helper = signers.account6;
    ...
    it("Should create raffle", async () => {
        ...
        deadline = Number(raffle.raffleData.deadline);
        ...
        expect(Number(raffle.state)).to.equal(3); // Raffle = 3
        expect(deadline).to.be.lt(now + 120).and.gt(now);
    });
    ...
    it("Should end raffle and send NFT to winner", async () => {
        ...
        // Wait until deadline
        const timeUntilDeadline = deadline - (await latestTimestamp());
        console.log(`\ttime until deadline: ${timeUntilDeadline} secs.`);
        await advanceTime(timeUntilDeadline + 1);
    ...
    it("Should create raffle and end it without participants", async () => {
        ...
        const raffle = await marketUtil.fetchPosition(raffleId);
        console.log("\tadvancing to deadline...");
        await advanceTime(Number(raffle.raffleData.deadline) - (await latestTimestamp()) + 1);
        console.log("\tDeadline reached");
        ...
        expect(Number(midTokenPositions[0].state)).to.equal(0); // Available = 0
        expect(Number(midTokenPositions[1].state)).to.equal(3); // Raffle = 3
        expect(endTokenPositions.length).to.equal(1);
        expect(Number(endTokenPositions[0].state)).to.equal(0); // Avalilable = 0
    });
